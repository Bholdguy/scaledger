...
- [ ] Re-run the cross-document terminology check (field names, enum values, per-mint-not-per-wallet framing) across all planning docs and the actual codebase.
- [ ] Confirm `.env.example` has no real secrets committed alongside it (only placeholder/blank values).
- [ ] Confirm the deployed/demo build's `APP_MODE` and the on-screen label agree with each other.
- [ ] Time-check against the Fri Sep 18, 4:00pm ET deadline with buffer for submission logistics, not just code-complete.
