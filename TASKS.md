...
- [x] **Found, flagged, not fixed (deliberately, given "no new features" scope for this pass):** the inline detail panel shows only the two computed ratios — the real tx signature, dividend amount, and `source_mode` label (required inline by PRD.md Step 6's own DoD wording) are only in the exported CSV/JSON, not the panel. `DEMO.md`'s script and rehearsed Q&A were rewritten to disclose this honestly rather than claim it's shown live. Left as a known, disclosed gap rather than risk a late UI change.
