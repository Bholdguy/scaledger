...
4. **Scaledger detects the real historical rebase event** — the mint's actual `UpdateMultiplierData` instruction, not a wallet balance heuristic. *On screen:* Fact (b)'s provenance line shows the real, truncated transaction signature as a link to Solana Explorer, plus the effective date. *Narration:* "This is a mechanism that only exists on Solana to check — Token-2022's Scaled UI Amount extension puts the rebase on the mint account itself, in the open, which is why we can point at one instruction — that link right there — and say 'this is the rebase' instead of guessing from a balance change."
5. **It fetches the real corporate-action size** for that dividend. *On screen:* Fact (a)'s provenance line literally shows "Manually sourced — dividend $0.50/share, prior close $97.33" with a link to the source. *Narration:* "That 'manually sourced' label isn't decoration — the xStocks API we hoped to use returned an auth error within our time-box, so we're using Strategy's own public dividend record instead, and the badge says so rather than pretending it's live."
...
8. **Presenter clicks Export (CSV or JSON)** — the same signature, dividend amount, prior close price, and `source_mode`/`source_url` shown inline now also appear together in one file, plus the raw ratios and timestamps, everything needed to independently verify the claim by hand.
